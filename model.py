...
def generate_name_greedy(rnn, charset):
  char = START_CHAR
  name = ''
  hidden = rnn.init_hidden()
  while char != END_CHAR:
...
def generate_name_optimal(rnn, charset, search_p=0.001):
    ...
    for j in range(n_char):
      ...
      if new_seq_p > search_p:
        ...
        if new_char == END_CHAR:
          if y[0].rstrip(END_CHAR).lstrip(START_CHAR) not in english_words:
